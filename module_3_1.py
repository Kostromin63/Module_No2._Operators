...
# Цель: применить на практике начальные знания о пространстве имён и оператор global. Закрепить навыки
# из предыдущих модулей.
#
...
def string_info(_str=''):
    count_calls()
    return len(_str), _str.upper(), _str.lower()


def is_contains(_str, _list):
    count_calls()
    for i in _list:
        if _str.lower() == i.lower():
            return True
