...
class Shop:
    def __init__(self):
        self.__file_name = 'ShopProdukt.txt'
    ...
    def add(self, *produkts):
        file_txt = self.get_products()
        new_produkt = ''
        for i in produkts:
            if str(i.name) in file_txt or str(i.name) in new_produkt:
                print(f'Продукт {i.name} уже есть в магазине')
                continue
            else:
                produkt_str = '\n' + i.__str__()
                new_produkt = new_produkt + produkt_str
        file = open(self.__file_name, "a")
        file.write(new_produkt)
        file.close()
