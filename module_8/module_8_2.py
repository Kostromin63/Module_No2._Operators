...
def calculate_average(numbers):
    """
    Должна принимать коллекцию numbers и возвращать: среднее арифметическое всех чисел.
    В нутри для подсчёта суммы используйте функцию personal_sum написанную ранее.
    ...
    """
    try:
        number_of_arguments = len(numbers)
    except TypeError:
        print(f'В numbers записан некорректный тип данных')
        return None
    tuple_sum_incor = personal_sum(numbers)
    sum = tuple_sum_incor[0]
    incor = tuple_sum_incor[1]
    resalt = 0
    if sum == 0:
        return resalt
    try:
        resalt = sum/(number_of_arguments-incor)
